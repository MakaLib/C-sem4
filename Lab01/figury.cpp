...
int main() {
  ...
  wypisz(t); // ma wypisac: Trojkat o bokach: 3 2 4
  wypisz(ko);// ma wypisac: Kolo o promieniu: 3

  Punkt* b = new Kwadrat(7); //Zmienione "Ksztalt" na Punkt z racji innej nazwy klasy bazowej
  wypisz(*b);
  ...
  const Punkt* wszystkie[] = {&k, &k, &ko, &t, &ko}; // obiekty powtorzone celowo
  
  for ( int idx = 0; idx < sizeof(wszystkie)/sizeof(Punkt*); ++idx ) {
    const Punkt* k = wszystkie[idx];
    k->wypisz(std::cout); 
